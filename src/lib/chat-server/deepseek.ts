...
const objectContract = {
  ...
  payload: "object. For scheduled cameraman capture, include payload.agent = \"cameraman\" and payload.recordingMode = \"cameraman_highlight\". For interval room watching, include payload.agent = \"watcher\" and payload.recordingMode = \"watcher_interval\". For baby care logs, use helper.teamMemberId = \"baby_logger\". For homekeeper reminders, use payload.agent = \"homekeeper\"; do not use \"reminder\" as an agent id.",
};
...
const responseContract = {
  ...
  helper: "optional, only when a helper takes a task: { teamMemberId: cameraman|watcher|companion|coach|homekeeper|baby_logger, name: string, reply: string, cards: [card], objectsToCreate: [object] }",
